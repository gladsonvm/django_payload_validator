...
class BaseValidatorView(View):
    ...
    def __init__(self, **kwargs):
        """
        Overriding django's base view to  ake sure that all necessary parameters are either set or provided.
        """
    ...
    def _check_attrs(self):
        for class_attr in self.required_class_attrs:
            if not self.hasattr(class_attr):
                raise ('{class_attr} is a required property for {class_name}'.format(class_attr=class_attr, class_name=self.__class__.__name__))
